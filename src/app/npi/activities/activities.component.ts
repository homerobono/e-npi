...
export class ActivitiesComponent implements OnInit {
    ...
    getModelActivityEndDate(activityLabel: String): Date {
        ...
        if (activity) {
            //console.log('activity: ' + activityLabel + ' -> ', greatestDate)
            if (activity.closed)
                return new Date(activity.signature.date)
            else
    ...
    fillFormData() {
        ...
        this.activitiesFormArray.controls.forEach(activityControl => {
            ...
            if (!activity.term && activity.signature && activity.signature.date)
                activityControl.patchValue({
                    term: (Math.floor(activityControl.get("endDate").value / DAYS) - Math.floor(activityControl.get("startDate").value / DAYS))
                })
    ...
    toggleFields(edit: Boolean) {
        this.activitiesFormArray.controls.forEach(control => {
            if (edit && this.npi.stage < 4) {
                ...
                control.get("activity").enable()
                if (this.npiComponent.user.level >= 1)
                    control.get("responsible").enable()
            }
            else
    ...
    fieldHasAnnex(activity: FormGroup) {
        ...
        return (this.uploadService.uploaders[`activities.${field}`] &&
            this.uploadService.uploaders[`activities.${field}`].queue &&
            this.uploadService.uploaders[`activities.${field}`].queue.length) ||
